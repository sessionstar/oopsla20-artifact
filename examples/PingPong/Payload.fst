...
let bytes_of_payload (payload:payload) : ML B.bytes =
    let header : B.bytes =
...
let payload_of_bytes (b:B.bytes) : ML payload =
...
(*
val involutive : (p:payload) -> Lemma
    (requires (exists (b:B.bytes). bytes_of_payload p = b))
    (ensures (payload_of_bytes (bytes_of_payload p) = p))
let involutive p = ()
 *)
